...
class LsController
{
    public function getCorrespondingLanguagesForCurrentRootPage($pageID = false)
    {
        ...
        while ($objRootPagesWithSameDomain->next()) {
            ...
            $obj_pageModel = \PageModel::findByAlias('regular' !== $objPage->type || $objPage->language !== $objRootPagesWithSameDomain->language ? $objRootPagesWithSameDomain->row()['alias'] : $objPage->row()['alias']);
            ...
            if (!\in_array($objRootPagesWithSameDomain->language, $languagesForCurrentDomain, true)) {
                $languagesForCurrentDomain[$objRootPagesWithSameDomain->language] = [
